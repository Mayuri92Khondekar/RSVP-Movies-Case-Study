...
select
		...
		SUM(CASE
        WHEN worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS worlwide_gross_income_null,
		SUM(CASE
        WHEN languages IS NULL THEN 1 ELSE 0 END) AS languages_null,
		SUM(CASE 
        WHEN production_company IS NULL THEN 1 ELSE 0 END) AS production_company_null
...
-- Type your code below:
SELECT country, COUNT(date_published) AS count_movies, year 
FROM movie
...
select genre as unique_genre_list,
RANK() OVER (ORDER BY genre) as No_of_unique_genre
FROM genre
GROUP BY genre;
...
select genre,count(genre) as no_of_genre
from movie as m
...
/* Output format:
+---------------+-------------------+---------------------+----------------------+-----------------+-----------------+
| |	min_avg_rating|	min_total_votes   |	max_total_votes 	 |min_median_rating|min_median_rating|
+---------------+-------------------+---------------------+----------------------+-----------------+-----------------+
|		0		|			5		|	       177		  |	   2000	    		 |		0	       |	8			 |
+---------------+-------------------+---------------------+----------------------+-----------------+-----------------+*/
-- Type your code below:
...
group by title, genre, median_rating
...
SELECT n.name AS director_name, COUNT(DISTINCT m.id) AS movie_count
FROM names AS n
...
on g.movie_id = r.movie_id
where genre = "Thriller"
group by movie_name, g.genre, r.avg_rating
...
GROUP BY actress_name, total_votes, avg_rating
order by movie_count
LIMIT 3;
